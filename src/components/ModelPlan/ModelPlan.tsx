...
const ModelPlan: React.FC = () => {
    ...
    const options = Object.keys(fullTables).map((major) => ({ value: major, label: major }));
    ...
    const renderTable = (major: string) => {
        ...
        return (
            <table className='model-plan'>
                <tbody>
                    <tr>
                        <td colSpan={3} style={{ textAlign: 'center', fontWeight: 'bold' }}>Semester One</td>
                    </tr>
                    {plans[major].map((course, index) => {
                        ...
                        return semesterText ? (
                            <tr key={index} style={isLastRow ? { fontWeight: 'bold' } : {}}>
                                <td colSpan={3} style={{ textAlign: 'center', fontWeight: 'bold' }}>{courseText}</td>
                            </tr>
                        ) : (
                            <tr key={index} style={isLastRow ? { fontWeight: 'bold' } : {}}>
                                {courseSpan > 0 && <td colSpan={courseSpan}>{courseText}</td>}
                                {descriptionSpan > 0 && <td colSpan={descriptionSpan}>{descriptionText}</td>}
                                {creditsSpan > 0 && <td colSpan={creditsSpan}>{creditsText}</td>}
                            </tr>
                        );
                    })}
                </tbody>
